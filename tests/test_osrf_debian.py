...
def test_only_amd64_and_arm64_are_queried(jetty):
    """i386 is unsupported and armhf production is being retired.

    What is left in the index for either is a leftover, and a leftover read as
    evidence that an architecture is built is what turns a deliberate drop into
    a reported gap.
    """
    http = FakeHttpClient()
    OsrfDebianSource(http).fetch(jetty)
    queried = {u.split("/binary-")[1].split("/")[0] for u in http.requested}
    assert queried == {"amd64", "arm64"}


def test_every_distro_is_queried_for_every_architecture(jetty):
    """No architecture is scoped to a subset of releases any more, so the
    distro list stays free to follow gz-collections.yaml on its own.
    """
    http = FakeHttpClient()
    OsrfDebianSource(http).fetch(jetty)
    seen = {(u.split("/dists/")[1].split("/")[0], u.split("/binary-")[1].split("/")[0])
            for u in http.requested}
    distros = {distro for distro, _ in seen}
    assert seen == {(d, a) for d in distros for a in ("amd64", "arm64")}
